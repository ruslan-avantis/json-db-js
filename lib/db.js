...
class DataBase {
	...
	insert(data) {
		...
		if (Array.isArray(data) && data.length >= 1) {
			ids = this.#insertArray(data)
		} else if (typeof data == 'object') {
			let arr = []
			arr.push(data)
			ids = this.#insertArray(arr)
		} else {
	...
	*/
	...
	/** Filter function for array.filter() in columns()
	 * @return boolean
	 */
	columnsPending() {
				...
				for(let key in pending) {
					...
					if (this.field) {
						...
						if (value_is_array && this.op == 'NOT IN') {
							...
							if (typeof value != 'object') value = value.toLowerCase()
							if (typeof this.value != 'object') this.value = this.value.toLowerCase()
...
DataBase.Error = class extends DbJsonError {
