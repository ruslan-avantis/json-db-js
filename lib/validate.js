...
class ValidateData {
	...
	constructor(settings = {}) {
		...
		*/
		this.defined_types = ['integer', 'boolean', 'double', 'string', 'array', 'object']
	...
	static table(table_name, settings = {}) {
		const res = new ValidateData(settings)
		res.table_name = table_name
		res.Config = TableConfig.table(table_name, settings)
		res.Data = TableData.table(table_name, settings)
		if (res.Config.exists() === true) res.schema()
		return res
	}
	...
	field(field) {
		...
		if (array_fields && includes) {
			...
			if (this.settings.console_error === true) {
				...
				throw new DbJsonError(err, field)
			}
		}
	}
	...
	type(field, value) {
		...
		if (type) {
			...
			if (!types[type]) {
				err = `Wrong data ! --> type: ${type} --> field: ${field} --> value: ${value} --> types: ${JSON.stringify(types)}`
			}
		} else err = `Field: ${field} --> type: ${type} does not exist`
